...
newman run \
  ...
  --env-var "HOST=http://192.168.112.126/provider-catalog-server/cp" \
...
curl --location "http://192.168.112.126/consumer/cs/api/identity/v1alpha/participants/" \
...
curl --location "http://192.168.112.126/provider/cs/api/identity/v1alpha/participants/" \
--header 'Content-Type: application/json' \
--header "x-api-key: $API_KEY" \
--data "$DATA_PROVIDER"
